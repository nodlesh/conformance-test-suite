...
export class BaseAgent {
  ...
  constructor(config: AgentConfiguration) {
    ...
    if (config.mediatorURL === undefined) {
      throw new Error("Mediator URL is required");
    }
    config.name = config.name || "GAN Verifier Demo";
    this._config = config;
    this._id = uuidv4();
    this._app = undefined; //express();
    this._agent = undefined;
  }

  get config(): AgentConfiguration {
    return this._config;
  }
  ...
  async init() {
    try {
      const modules = this.getAgentModules({
        indyNetworks: [],
        mediatorInvitationUrl: this.config.mediatorURL,
        txnCache: {
          capacity: 1000,
      ...
      const agent = new Agent({
        ...
        config: {
          ...
          walletConfig: this.config.config?.walletConfig || {
      ...
      // const wsInboundTransport = new WsInboundTransport({ port });
      ...
      console.error(`Failed to initialize agent (${this.config.name}):`, e);
      throw e;
    }
  }
  ...
  getAgentModules({}: GetAgentModulesOptions) {
  ...
  async getConnectionRecord(outOfBandId: string): Promise<ConnectionRecord> {
